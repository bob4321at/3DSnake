...
var Test_Shader = `//kage:unit pixels
	...
	func Fragment(targetCoords vec4, srcPos vec2, _ vec4) vec4 {
		...
		below := imageSrc0At(vec2(srcPos.x+(targetCoords.x/64)-(9), srcPos.y+(targetCoords.y/64)-(6)))
		closebelow := imageSrc0At(vec2(srcPos.x+(targetCoords.x/128)-(9), srcPos.y+(targetCoords.y/128)-(6)))
